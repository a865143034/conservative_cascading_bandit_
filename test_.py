...
class c3_movielens_rng:
    ...
    def slot(self):
        ...
        return {arm: np.outer(self.U[self.user], self.V[arm]).flatten() for arm in self.arms if
                exc[0, arm] == 0} if len([arm for arm in self.arms if exc[0, arm] == 0]) > self.K + 2 else self.slot()
    ...
    def load(self):
        ...
        self.A = sparse.coo_matrix((np.ones(len(cox)), (cox, coy)),
                                   shape=(max(list(self.ctrh) + cox) + 1, max(list(movies) + coy) + 1),
                                   dtype=np.float32)
        ...
        self.users = [x[1] for x in sorted([(overlap(self.ctrh[user], self.arms), user) for user in self.ctrh if
                                            self.L * self.baseline[0] < overlap(self.ctrh[user], self.arms) < self.L *
                                            self.baseline[1]])]
...
class c3_Lmovielens_rng(c3_movielens_rng):
    ...
    def slot(self):
        self.user = random.sample(self.users, 1)[0]
        exc = self.A.getrow(self.user)
        return {arm: self.V[arm].flatten() for arm in self.arms if exc[0, arm] == 0} if len(
            [arm for arm in self.arms if exc[0, arm] == 0]) < self.K + 2 else self.slot()